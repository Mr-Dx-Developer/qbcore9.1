...
QBCore.Functions.CreateCallback('qb-policehub:officers:count', function(source, cb)
    local count = 0
    local policelist = {}
...
QBCore.Functions.CreateCallback('qb-policehub:officer:location', function(source, cb)
    local count = 0
    local policelist = {}
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
...
RegisterServerEvent('qb-policehub:get:officers')
AddEventHandler('qb-policehub:get:officers', function()
    local src = source
    local cops = 0
    local data = {}
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
    ...
    TriggerClientEvent('qb-policehub:send:officers', src, data, cops)
end)

RegisterServerEvent('qb-policehub:SendPanicToPlayers')
AddEventHandler('qb-policehub:SendPanicToPlayers', function(coords, grade, name)
    local src = source
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        ...
        if Player.PlayerData.job.name == "police" then
            ...
            TriggerClientEvent("qb-policehub:SetPanicBlip", Player.PlayerData.source, coords)
        end
    end
end)

RegisterServerEvent('qb-policehub:officer:change:callsign')
AddEventHandler('qb-policehub:officer:change:callsign', function(id, callsign)
