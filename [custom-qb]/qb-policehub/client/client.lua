...
RegisterCommand('policehub', function(source, args)
  if args[1] == "0" then
    if not menu_opened then
      menu_opened = true
      toggleNuiFrame(true)
      TriggerServerEvent('qb-policehub:get:officers')
    else
      menu_opened = false
...
RegisterNUICallback('hideFrame', function(_, cb)
  toggleNuiFrame(false)
  TriggerEvent('qb-policehub:close:all')
  menu_opened = false
  cb({})
end)

RegisterNetEvent('qb-policehub:close:all')
AddEventHandler('qb-policehub:close:all', function()
    SendReactMessage('closeall', {})
end)

local cops = 0
RegisterNetEvent('qb-policehub:send:officers')
AddEventHandler('qb-policehub:send:officers', function(users, cops)
    SendReactMessage('open', { users = users, count = tostring(cops) })
    cops = users.count
end)

RegisterNUICallback('Get-officers-count', function(data, cb)
  QBCore.Functions.TriggerCallback('qb-policehub:officers:count', function(count2)
...
RegisterNUICallback("Use-Panic-Button", function()
  ...
  TriggerServerEvent('qb-policehub:SendPanicToPlayers', playerCoords, playergrade, playername, playerped)
end)

local PanicBlipData = nil

RegisterNetEvent('qb-policehub:SetPanicBlip')
AddEventHandler('qb-policehub:SetPanicBlip', function(PlayerCoords)
...
RegisterNUICallback("Change-Officer-Callsign", function(data, cb)
  ...
  TriggerServerEvent('qb-policehub:officer:change:callsign', playerid, playernewcallsign)
  cb({})
end)
