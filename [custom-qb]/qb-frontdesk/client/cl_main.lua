            ...
            options = {
                {
                    type = "client",
                    action = function()
                        TriggerEvent("qb-frontdesk:client:OpenFrontDesk", closestDesk)
                    end,
                    icon = "fas fa-desktop",
                    label = "Front Desk",
                },
...
-- Front Desk Menu
RegisterNetEvent('qb-frontdesk:client:OpenFrontDesk',function(job)
    if PlayerJob.name == job then
        local FrontDeskMenu = {
            ...
            },
            {
                ...
                params = {
                    event = 'qb-frontdesk:client:ToggleDuty',
                    args = job
                }
            },
            {
                ...
                params = {
                    event = 'qb-frontdesk:client:OpenAssistanceMenu',
                    args = job
                }
            },
            {
...
-- Toggle Duty Event
RegisterNetEvent('qb-frontdesk:client:ToggleDuty',function(job)
    if job == 'police' then
...
-- Assistance Menu
RegisterNetEvent('qb-frontdesk:client:OpenAssistanceMenu',function(job)
    ...
    AssistanceMenu[#AssistanceMenu + 1] = {
        ...
        params = {
            event = 'qb-frontdesk:client:OpenFrontDesk'
        }
    }
...
RegisterNetEvent('qb-frontdesk:client:RequestAssistance',function(data)
    QBCore.Functions.TriggerCallback('qb-frontdesk:server:CooldownCheck', function(alert)
        if alert then
            if Config.Dispatch == "default" then
                if data.job == 'police' then
                    if CurrentCops >= Config.Locations[data.job].Required then
                        ...
                        TriggerServerEvent('qb-frontdesk:server:AlertCooldown', data.job, true)
                    else
