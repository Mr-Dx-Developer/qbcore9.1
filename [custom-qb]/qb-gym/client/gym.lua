...
local function CheckTraining()
	if resting == true then
        QBCore.Functions.Notify('Are you resting', 'primary')
		resting = false
		Wait(15000)
		training = false
	end
	if resting == false then
        QBCore.Functions.Notify('Now you can do the exercise again', 'success')
	end
end
...
function returnTicketMenu()
    exports['qb-menu']:openMenu({
        ...
        {
            header = "Terminate Membership",
			txt = "End of membership",
			params = {
...
function displayTime()
    TriggerEvent('cd_drawtextui:ShowUI', 'show', "Time to visit the gym</br> "..minutes..":"..seconds)
end
...
CreateThread(function()
	...
	AddTextComponentString('Fitness room')
							...
							if IsControlJustReleased(0, 38) then
								if gotTicket then
									if training == false then
										...
										QBCore.Functions.Notify('Starting the exercise', 'success')
										Wait(1000)					
										...
										--	print(Config.Locations[k].skill, Config.Locations[k].SkillAddQuantity)
											QBCore.Functions.Notify('You need to rest for 10 seconds before doing another exercise', 'error')
											training = true
											resting = true
											CheckTraining()
										elseif membership == false then
											QBCore.Functions.Notify('You dont have a subscription to do this exercise', 'error')
										end
									...
									wait = 15000
									QBCore.Functions.Notify("the subscription is no longer valid", "error")
								end
							end
						end
					end
