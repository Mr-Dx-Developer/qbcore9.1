...
RegisterServerEvent('qb-impound:server:VehicS', function()
    local src = source
...
RegisterServerEvent('qb-impound:server:ParkingVio', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('qb-impound:server:PDIm', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" then
...
-- only buy from discord.gg/qbcoreframework -- QBCore#4011