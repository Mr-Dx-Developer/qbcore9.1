...
RegisterNetEvent('qb-gunheist:crate:clearcrate', function()
    crate = nil
end)
...
function OpenCrate(entity, crate, item, amount)
    ...
    TriggerServerEvent('qb-gunheist:crate:deleteCrate', netId)

    TriggerServerEvent("qb-gunheist:server:ItemHandler", "add", item, amount)                           
...
-- Apply Natives
RegisterNetEvent("qb-gunheist:crate:applyNatives")
AddEventHandler("qb-gunheist:crate:applyNatives", function(netId)
...
function CrateDrop(item, amount, planeSpawnDistance, dropCoords)
    CreateThread(function()
        ...
        SetTimeout(Config.TimeUntilDrop * 60 * 1000, function()            	               
            ...
            TriggerServerEvent("qb-gunheist:crate:spawnCrate", crateSpawn, item, amount)
            while crate == nil do
                Wait(250)
            end
...
RegisterNetEvent("qb-gunheist:crate:createQbTarget")
AddEventHandler("qb-gunheist:crate:createQbTarget", function(crate, item, amount)
...
-- Start the AirDrop
RegisterNetEvent("qb-gunheist:client:StartDrop", function(item, amount, roofCheck, planeSpawnDistance, dropCoords)
    CreateThread(function()          
...
RegisterNetEvent('qb-gunheist:crate:destroyzone', function()
...
-- Create the AirDrop
RegisterNetEvent("qb-gunheist:client:CreateDrop", function(useditem, roofCheck, planeSpawnDistance)
    local playerCoords = GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 10.0, 0.0)        
    local item, amount = GetRandomItemData(useditem)
    QBCore.Functions.TriggerCallback("qb-gunheist:server:cooldown", function(cooldown) 
        if not cooldown then
            QBCore.Functions.TriggerCallback("qb-gunheist:server:getCops", function(CurrentCops)
                if CurrentCops >= Config.RequiredCops then
                    TriggerServerEvent('qb-gunheist:server:startCooldown')              
                    QBCore.Functions.Notify(Config.Lang["pilot_contact"], "success", 10000)
                    PoliceAlert()    
                    TriggerServerEvent("qb-gunheist:server:ItemHandler", "remove", useditem, 1)
                    TriggerEvent("qb-gunheist:client:StartDrop", item, amount, roofCheck or false, planeSpawnDistance or 400.0, vector3(playerCoords.x, playerCoords.y, playerCoords.z))                
                else
