...
-- RegisterNetEvent('qb-smallresources:client:SeizeItems', function()
--     local closestPlayer, closestDistance = QBCore.Functions.GetClosestPlayer()
    
--     if closestPlayer == -1 or closestDistance >= 2.5 then
--         TriggerEvent('QBCore:Notify', "No player nearby or too far away.", "error")
--         return
--     end

--     local playerData = QBCore.Functions.GetPlayerData(closestPlayer)

--     if not playerData or not playerData.inventory then
--         TriggerEvent('QBCore:Notify', "Unable to retrieve player data for nearby player.", "error")
--         return
--     end

--     if not Config.SeizeItems or type(Config.SeizeItems) ~= "table" or not next(Config.SeizeItems) then
--         TriggerEvent('QBCore:Notify', "SeizeItems configuration is missing or empty.", "error")
--         return
--     end

--     for _, itemName in ipairs(Config.SeizeItems) do
--         local item = playerData.inventory[itemName]

--         if item and item.count > 0 then
--             TriggerServerEvent('QBCore:Server:RemoveItem', closestPlayer, itemName, 1)
--             TriggerEvent('QBCore:Notify', "You seized " .. itemName .. " from " .. GetPlayerName(closestPlayer), "success")
--         else
--             TriggerEvent('QBCore:Notify', GetPlayerName(closestPlayer) .. " does not have any " .. itemName, "error")
--         end
--     end
-- end)
...
RegisterNetEvent('qb-smallresources:client:SeizeItems', function()
    ...
    local itemNames = {"phone", "weapon_pistol", "tosti"}

    for _, itemName in ipairs(itemNames) do
        Player.Functions.GetItemByName(itemName, function(item)
...
RegisterNetEvent('consumables:client:UseParachute', function()
    ...
    QBCore.Functions.Progressbar("use_parachute", Lang:t('consumables.use_parachute_progress'), 5000, false, true, {
        ...
        TriggerEvent('qb-clothing:client:reloadOutfits', ParachuteData)
...
RegisterNetEvent('consumables:client:ResetParachute', function()
    if ParachuteEquiped then
        ...
        QBCore.Functions.Progressbar("reset_parachute", Lang:t('consumables.pack_parachute_progress'), 40000, false, true, {
            ...
            TriggerEvent('qb-clothing:client:reloadOutfits', ParachuteRemoveData)
            TaskPlayAnim(ped, "clothingshirt", "exit", 8.0, 1.0, -1, 49, 0, false, false, false)
