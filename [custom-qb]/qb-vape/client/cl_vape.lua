			...
			options = {
				{
					...
					event = "qb-vape:client:OpenCraft",
					label = "Craft Station",
				},
				{
					type = "client",
					icon = "fa-solid fa-basket-shopping",
					event = "qb-vape:client:supplies",
					label = "Vape Store",
				},
			},
		distance = 1.5
...
RegisterNetEvent("qb-vape:client:supplies", function()
	TriggerServerEvent("inventory:server:OpenInventory", "shop", "Vape", Config.Juice)
end)

RegisterNetEvent('qb-vape:client:OpenCraft', function()
    exports['qb-menu']:openMenu({
        ...
        },
        {
            ...
            params = {
                event = "qb-vape:client:craftVape"
            }
        },
		{
            id = 2,
            ...
            icon = "fa-solid fa-square-up-right",
            params = {
                event = "qb-vape:client:refillVape"
            }
        },
        {
            id = 3,
...
RegisterNetEvent("qb-vape:client:useVape", function(ItemData)
	...
	if not deadBozo then 
		if not isVaping then
			...
			QBCore.Functions.Progressbar("isVaping_lol", "Smoking vape..", 5000, false, false, {
				...
				TriggerServerEvent("qb-vape:server:syncSmoke", pedNet, pos)
				...
				TriggerServerEvent('qb-vape:server:updateVape', dragsLeft, dragsLeftData)
				isVaping = false
			end)
		else
...
RegisterNetEvent("qb-vape:client:syncSmoke", function(netPed, pos)
	local ped = PlayerPedId()
...
RegisterNetEvent('qb-vape:client:craftVape', function()
		...
		if glass then
			if elec then 
				...
				QBCore.Functions.Progressbar("vape_craft", "Crafting a vape..", 5000, false, false, {
					...
					TriggerServerEvent('qb-vape:server:makeVape')
				end)
			else
...
RegisterNetEvent('qb-vape:client:refillVape', function()
	...
	if gotVape then
		if gotJuice then
			...
			QBCore.Functions.Progressbar("vape_craft", "Refilling vape..", 5000, false, false, {
				...
				TriggerServerEvent('qb-vape:server:fillVape')
			end)
		else
