...
RegisterNetEvent("qb-vape:server:syncSmoke", function(pedNet, pos)
	for k, v in pairs(QBCore.Functions.GetPlayers()) do
		TriggerClientEvent("qb-vape:client:syncSmoke", v, pedNet, pos)
    end
end)
...
QBCore.Functions.CreateUseableItem("vape", function(source, item)
    ...
    TriggerClientEvent('qb-vape:client:useVape', src, item)
end)
...
RegisterNetEvent('qb-vape:server:updateVape', function(vapesLeft, data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('qb-vape:server:makeVape', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local info = {
        uses = 100
...
RegisterNetEvent('qb-vape:server:fillVape', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
