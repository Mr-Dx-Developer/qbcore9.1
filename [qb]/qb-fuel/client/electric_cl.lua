if Config.ElectricVehicleCharging then
    ...
    function SetElectricNozzle(state)
        ...
        elseif state == "pickup" then    
            TriggerEvent('qb-fuel:client:grabelectricnozzle')
                ...
                options = {
                    {
                        ...
                        event = 'qb-fuel:client:electric:ChargeVehicle',
                        args = {
    ...
    RegisterNetEvent('qb-fuel:client:electric:FinalMenu', function(purchasetype)
        local money = nil
            ...
            if Electricity then
                ...
                if (ElectricityAmount + finalfuel) >= 100 then
                    ...
                    if GlobalTax(ElectricityAmount * FuelPrice) + (ElectricityAmount * FuelPrice) <= money then
                        TriggerServerEvent('qb-fuel:server:electric:OpenMenu', ElectricityAmount, IsInGasStation(), false, purchasetype, FuelPrice)
                    else
            ...
            if Electricity then
                ...
                if (Electricity.amount + finalfuel) >= 100 then
                    ...
                    if GlobalTax(Electricity.amount * FuelPrice) + (Electricity.amount * FuelPrice) <= money then
                        TriggerServerEvent('qb-fuel:server:electric:OpenMenu', Electricity.amount, IsInGasStation(), false, purchasetype, FuelPrice)
                    else
                        QBCore.Functions.Notify(Lang:t("not_enough_money"), 'error', 7500)
                    end
                end
    ...
    RegisterNetEvent('qb-fuel:client:electric:SendMenuToServer', function()
        local vehicle = GetClosestVehicle()
                        ...
                        options = {
                            {
                                ...
                                icon = "fas fa-usd",
                                arrow = false, -- puts arrow to the right
                                event = "qb-fuel:client:electric:FinalMenu",
                                args = 'cash',
                            },
                            {
    ...
    RegisterNetEvent('qb-fuel:client:electric:ChargeVehicle', function(data)
                ...
                if Config.Ox.Progress then
                    if lib.progressCircle({
                        ...
                        if purchasetype == "cash" then
                            TriggerServerEvent('qb-fuel:server:PayForFuel', refillCost, purchasetype, FuelPrice, true)
                    ...
                    QBCore.Functions.Progressbar("charge-car", Lang:t("prog_electric_charging"), time, false, true, {
                        ...
                        refueling = false
                        if not Config.RenewedPhonePayment or purchasetype == 'cash' then TriggerServerEvent('qb-fuel:server:PayForFuel', refillCost, purchasetype, FuelPrice, true) end
                        local curfuel = GetFuel(vehicle)
                        local finalfuel = (curfuel + fuelamount)
                        if finalfuel > 99 and finalfuel < 100 then
                            SetFuel(vehicle, 100)
    ...
    RegisterNetEvent('qb-fuel:client:grabelectricnozzle', function()
        local ped = PlayerPedId()
    ...
    RegisterNetEvent('qb-fuel:client:electric:RefuelMenu', function()
        if Config.RenewedPhonePayment then
            if not RefuelPossible then 
                TriggerEvent('qb-fuel:client:electric:SendMenuToServer')
            else 
                if Config.RenewedPhonePayment then
                    if not Cancelledrefuel and not RefuelCancelled then
                        if RefuelPossibleAmount then
                            ...
                            TriggerEvent('qb-fuel:client:electric:ChargeVehicle', purchasetype, fuelamounttotal)
                        else
    ...
    if Config.RenewedPhonePayment then
        RegisterNetEvent('qb-fuel:client:electric:phone:PayForFuel', function(amount)
            ...
            if success then
                if QBCore.Functions.GetPlayerData().money['bank'] <= (GlobalTax(amount) + amount) then
                    ...
                    TriggerServerEvent('qb-fuel:server:PayForFuel', total, "bank", FuelPrice, true)
                    RefuelPossible = true
        ...
        options = {
            {
                num = 1,
                type = "client",
                event = "qb-fuel:client:grabelectricnozzle",
                icon = "fas fa-bolt",
