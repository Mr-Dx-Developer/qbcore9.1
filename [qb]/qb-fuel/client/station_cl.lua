if Config.PlayerOwnedGasStationsEnabled then -- This is so Player Owned Gas Stations are a Config Option, instead of forced. Set this option in shared/config.lua!
                ...
                options = {
                    {
                        ...
                        action = function()
                            TriggerEvent('qb-fuel:stations:openmenu', CurrentLocation)
                        end,
                    },
                },
                distance = 2.0
    ...
    -- Events
    RegisterNetEvent('qb-fuel:stations:updatelocation', function(updatedlocation)
    ...
    RegisterNetEvent('qb-fuel:stations:client:buyreserves', function(data)
        local location = data.location
        local price = data.price
        local amount = data.amount
        TriggerServerEvent('qb-fuel:stations:server:buyreserves', location, price, amount)
    ...
    RegisterNetEvent('qb-fuel:station:client:initiatefuelpickup', function(amountBought, finalReserveAmountAfterPurchase, location)
        if amountBought and finalReserveAmountAfterPurchase and location then
            ...
            if SpawnPickupVehicles() then
                                    ...
                                    if GetVehiclePedIsIn(ped, false) == spawnedDeliveryTruck then
                                        ...
                                        if not alreadyHasTruck then
                                            ...
                                            if not CreatedEventHandler then
                                                        ...
                                                        while true do
                                                            ...
                                                            if IsControlJustReleased(2, 38) then
                                                                ...
                                                                if distBetweenTruckAndTrailer > 10.0 then
                                                                    ...
                                                                    TriggerServerEvent('qb-fuel:station:server:fuelpickup:finished', ReservePickupData.location)
                                                                    -- Remove Handler
                                                ...
                                                locationSwapHandler = AddEventHandler('qb-fuel:stations:updatelocation', function(location)
                ...
                TriggerServerEvent('qb-fuel:station:server:fuelpickup:failed', location)
            end
        else
            if Config.FuelDebug then
                print("An error has occurred. The amountBought / finalReserveAmountAfterPurchase / location is nil: `qb-fuel:station:client:initiatefuelpickup`")
            end
        end
    end)

    RegisterNetEvent('qb-fuel:stations:client:purchaselocation', function(data)
        local location = data.location
        local CitizenID = QBCore.Functions.GetPlayerData().citizenid
        CanOpen = false
        Wait(5)
        QBCore.Functions.TriggerCallback('qb-fuel:server:locationpurchased', function(result)
            if result then
        ...
        if not IsOwned then
            TriggerServerEvent('qb-fuel:server:buyStation', location, CitizenID)
        elseif IsOwned then
            QBCore.Functions.Notify(Lang:t("station_already_owned"), 'error', 7500)
        end
    end)

    RegisterNetEvent('qb-fuel:stations:client:sellstation', function(data)
        local location = data.location
        local SalePrice = data.SalePrice
        local CitizenID = QBCore.Functions.GetPlayerData().citizenid
        CanSell = false
        Wait(5)
        QBCore.Functions.TriggerCallback('qb-fuel:server:isowner', function(result)
            if result then
        ...
        if CanSell then
            ...
            TriggerServerEvent('qb-fuel:stations:server:sellstation', location)
    ...
    RegisterNetEvent('qb-fuel:stations:client:purchasereserves:final', function(location, price, amount) -- Menu, seens after selecting the "purchase reserves" option.
                    ...
                    options = {
                        {
                            ...
                            event = 'qb-fuel:stations:client:buyreserves',
                            args = {
                                location = location,
                                price = price,
                                amount = amount,
    ...
    RegisterNetEvent('qb-fuel:stations:client:purchasereserves', function(data)
        local CanOpen = false
        local location = data.location
        QBCore.Functions.TriggerCallback('qb-fuel:server:isowner', function(result)
            local CitizenID = QBCore.Functions.GetPlayerData().citizenid
                ...
                if reserves then
                    ...
                    if (Reservebuyamount + Currentreserveamount) > Config.MaxFuelReserves then
                        ...
                        if math.ceil(GlobalTax(Reservebuyamount * Config.FuelReservesPrice) + (Reservebuyamount * Config.FuelReservesPrice)) <= bankmoney then
                            ...
                            TriggerEvent("qb-fuel:stations:client:purchasereserves:final", location, price, amount)
    ...
    RegisterNetEvent('qb-fuel:stations:client:changefuelprice', function(data)
        CanOpen = false
        local location = data.location
        QBCore.Functions.TriggerCallback('qb-fuel:server:isowner', function(result)
            local CitizenID = QBCore.Functions.GetPlayerData().citizenid
            ...
            if Config.Ox.Input then
                ...
                if fuelprice then
                    ...
                    if NewFuelPrice > Config.MaxFuelPrice then
                        ...
                        TriggerServerEvent("qb-fuel:station:server:updatefuelprice", NewFuelPrice, CurrentLocation)
                    end
                end
            else
    ...
    RegisterNetEvent('qb-fuel:stations:client:sellstation:menu', function(data) -- Menu, seen after selecting the Sell this Location option.
        local location = data.location
        local CitizenID = QBCore.Functions.GetPlayerData().citizenid
        QBCore.Functions.TriggerCallback('qb-fuel:server:isowner', function(result)
            if result then
        ...
        if CanOpen then
            ...
            if Config.Ox.Menu then
                    ...
                    options = {
                        {
                            ...
                            icon = "fas fa-usd",
                            arrow = false, -- puts arrow to the right
                            event = 'qb-fuel:stations:client:sellstation',
                            args = {
                                location = location,
                                SalePrice = SalePrice,
                            }
                ...
                TriggerServerEvent("qb-fuel:stations:server:stationsold", location)
            else
                exports['qb-menu']:openMenu({
                    {
    ...
    RegisterNetEvent('qb-fuel:stations:client:changestationname', function() -- Menu for changing the label of the owned station.
        CanOpen = false
        QBCore.Functions.TriggerCallback('qb-fuel:server:isowner', function(result)
            local CitizenID = QBCore.Functions.GetPlayerData().citizenid
            if result then
        ...
        if CanOpen then
            ...
            if Config.Ox.Input then
                ...
                if NewName then
                    ...
                    Wait(100)
                    TriggerServerEvent("qb-fuel:station:server:updatelocationname", NewName, CurrentLocation)
                end
            else
    ...
    RegisterNetEvent('qb-fuel:stations:client:managemenu', function(location) -- Menu, seen after selecting the Manage this Location Option.
                    ...
                    options = {
                        {
                            ...
                            event = 'qb-fuel:stations:client:purchasereserves',
                            args = {
                                location = location,
                            },
                            metadata = {
                        ...
                        },
                        {
                            ...
                            icon = "fas fa-usd",
                            arrow = false, -- puts arrow to the right
                            event = 'qb-fuel:stations:client:changefuelprice',
                            args = {
                                location = location,
                            },
                            metadata = {
                        ...
                        },
                        {
                            ...
                            icon = "fas fa-usd",
                            arrow = false, -- puts arrow to the right
                            event = 'qb-fuel:stations:client:managefunds'
                        },
                        {
                            ...
                            icon = "fas fa-pen",
                            arrow = false, -- puts arrow to the right
                            event = 'qb-fuel:stations:client:changestationname',
                        ...
                        },
                        {
                            ...
                            icon = "fas fa-usd",
                            arrow = false, -- puts arrow to the right
                            event = 'qb-fuel:stations:client:sellstation:menu',
                            args = {
                                location = location,
                            },
                        },
    ...
    RegisterNetEvent('qb-fuel:stations:client:managefunds', function(location) -- Menu, seen after selecting the Manage this Location Option.
        QBCore.Functions.TriggerCallback('qb-fuel:server:isowner', function(result)
            local CitizenID = QBCore.Functions.GetPlayerData().citizenid
            if result then
                if Config.FuelDebug then print("The Location: "..CurrentLocation.." is owned by ID: "..CitizenID) end
                    ...
                    options = {
                        {
                            ...
                            arrow = false, -- puts arrow to the right
                            event = 'qb-fuel:stations:client:WithdrawFunds',
                            args = {
                                location = location,
                            }
                        },
                        {
                            ...
                            arrow = false, -- puts arrow to the right
                            event = 'qb-fuel:stations:client:DepositFunds',
                            args = {
                                location = location,
                            }
                        },
                        {
                            ...
                            arrow = false, -- puts arrow to the right
                            event = 'qb-fuel:stations:client:managemenu',
                            args = {
                                location = location,
                            }
                        },
    ...
    RegisterNetEvent('qb-fuel:stations:client:WithdrawFunds', function(data)
            ...
            if Config.Ox.Input then
                ...
                if Withdraw then
                    ...
                    if (StationBalance - WithdrawAmount) < 0 then
                        ...
                        TriggerServerEvent('qb-fuel:station:server:Withdraw', amount, location, StationBalance)
                    end
                end
            else
    ...
    RegisterNetEvent('qb-fuel:stations:client:DepositFunds', function(data)
            ...
            if Config.Ox.Input then
                ...
                if Deposit then
                    ...
                    if (DepositAmount) > bankmoney then
                        ...
                        TriggerServerEvent('qb-fuel:station:server:Deposit', amount, location, StationBalance)
                    end
                end
            else
    ...
    RegisterNetEvent('qb-fuel:stations:client:Shutoff', function(location)
        TriggerServerEvent("qb-fuel:stations:server:Shutoff", location)
    end)

    RegisterNetEvent('qb-fuel:stations:client:purchasemenu', function(location) -- Menu, seen after selecting the purchase this location option.
        local bankmoney = QBCore.Functions.GetPlayerData().money['bank']
        ...
        if Config.OneStationPerPerson == true then
            QBCore.Functions.TriggerCallback('qb-fuel:server:doesPlayerOwnStation', function(result)
                if result then
                ...
                options = {
                    {
                        ...
                        icon = "fas fa-usd",
                        arrow = true, -- puts arrow to the right
                        event = 'qb-fuel:stations:client:purchaselocation',
                        args = {
                            location = location,
                        },
                        metadata = {
    ...
    RegisterNetEvent('qb-fuel:stations:openmenu', function() -- Menu #1, the first menu you see.
            ...
            lib.registerContext({
                ...
                options = {
                    ...
                    },
                    {
                        ...
                        icon = "fas fa-usd",
                        arrow = false, -- puts arrow to the right
                        event = 'qb-fuel:stations:client:purchasemenu',
                        args = CurrentLocation,
                    ...
                    },
                    {
                        ...
                        icon = "fas fa-gas-pump",
                        arrow = false, -- puts arrow to the right
                        event = 'qb-fuel:stations:client:Shutoff',
                        args = CurrentLocation,
