...
local function FetchStationInfo(info)
	...
	QBCore.Functions.TriggerCallback('qb-fuel:server:fetchinfo', function(result)
		if result then
...
if Config.ShowNearestGasStationOnly then
	RegisterNetEvent('qb-fuel:client:updatestationlabels', function(location, newLabel)
	...
	CreateThread(function()
		if Config.PlayerOwnedGasStationsEnabled then
			TriggerServerEvent('qb-fuel:server:updatelocationlabels')
		end
		Wait(1000)
	...
	for station_id = 1, #Config.GasStations, 1 do
		...
		Stations[station_id]:onPlayerInOut(function(isPointInside)
			if isPointInside then
				...
				if Config.PlayerOwnedGasStationsEnabled then
					TriggerEvent('qb-fuel:stations:updatelocation', station_id)
				end
			else
				TriggerEvent('qb-fuel:stations:updatelocation', nil)
				inGasStation = false
			end
		end)
	end
...
if Config.RenewedPhonePayment then
	RegisterNetEvent('qb-fuel:client:phone:PayForFuel', function(amount)
		...
		if success then
			if QBCore.Functions.GetPlayerData().money['bank'] <= total then
				...
				TriggerServerEvent('qb-fuel:server:PayForFuel', total, "bank", FuelPrice, false, CachedFuelPrice)
				RefuelPossible = true
...
if Config.Ox.Menu then
	RegisterNetEvent('qb-fuel:client:OpenContextMenu', function(total, fuelamounttotal, purchasetype)
...
RegisterNetEvent('qb-fuel:client:RefuelMenu', function(type)
	if Config.FuelDebug then print("qb-fuel:client:refuelmenu") end
	if not type then type = nil end
	if Config.RenewedPhonePayment then
		if not RefuelPossible then 
			TriggerEvent('qb-fuel:client:SendMenuToServer', type)
		else
...
RegisterNetEvent('qb-fuel:client:grabnozzle', function()
	if Config.PlayerOwnedGasStationsEnabled then
		ShutOff = false
		Wait(50)
		QBCore.Functions.TriggerCallback('qb-fuel:server:checkshutoff', function(result)
			if result == true then
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		...
		if Config.TargetResource == 'ox_target' then
			exports.ox_target:removeGlobalVehicle('qb-fuel:options:1')
			exports.ox_target:removeGlobalVehicle('qb-fuel:options:2')
		end
...
RegisterNetEvent('qb-fuel:client:FinalMenu', function(purchasetype)
	if Config.FuelDebug then
		print('qb-fuel:client:FinalMenu', purchasetype)
	end
	if RefuelingType == nil then
		...
		if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel then
			if ReserveLevels and ReserveLevels < 1 then
		...
		if fuel then
			...
			if (fuelAmount + finalfuel) >= 100 then
				...
				if GlobalTax(fuelAmount * FuelPrice) + (fuelAmount * FuelPrice) <= money then
					TriggerServerEvent('qb-fuel:server:OpenMenu', fuelAmount, inGasStation, false, purchasetype, tonumber(FuelPrice))
				else
		...
		if fuel then
			...
			if (fuel.amount + finalfuel) >= 100 then
				...
				if GlobalTax(fuel.amount * FuelPrice) + (fuel.amount * FuelPrice) <= money then
					...
					TriggerServerEvent('qb-fuel:server:OpenMenu', fuel.amount, inGasStation, false, purchasetype, tonumber(FuelPrice))
				else
					QBCore.Functions.Notify(Lang:t("not_enough_money"), 'error', 7500)
				end
			end
...
RegisterNetEvent('qb-fuel:client:SendMenuToServer', function(type)
	local vehicle = GetClosestVehicle()
					...
					options = {
						{
							...
							arrow = false, -- puts arrow to the right
							onSelect = function ()
								TriggerEvent('qb-fuel:client:FinalMenu', 'cash')
							end,
						},
						{
							...
							arrow = false, -- puts arrow to the right
							onSelect = function ()
								TriggerEvent('qb-fuel:client:FinalMenu', 'bank')
							end,
						},
						{
							title = Lang:t("menu_header_close"),
		...
		if IsPlayerNearVehicle() then
				...
				while refueling do
					...
					if Cancelledrefuel then
						...
						if Config.RenewedPhonePayment and purchasetype == "bank" then
							...
							TriggerServerEvent("qb-fuel:server:phone:givebackmoney", MoneyToGiveBack)
							CachedFuelPrice = nil
						else
							TriggerServerEvent('qb-fuel:server:PayForFuel', refillCost, purchasetype, FuelPrice, false, CachedFuelPrice)
							CachedFuelPrice = nil
						end
						if RefuelingType == nil then
							if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel then
								TriggerServerEvent('qb-fuel:station:server:updatereserves', "remove", finalrefuelamount, ReserveLevels, CurrentLocation)
								if CachedFuelPrice ~= nil then
									if Config.FuelDebug then
										print("We have a cached price: $"..CachedFuelPrice..", we will credit this to the gas station.")
									end
									TriggerServerEvent('qb-fuel:station:server:updatebalance', "add", finalrefuelamount, StationBalance, CurrentLocation, CachedFuelPrice)
									CachedFuelPrice = nil
								else
									TriggerServerEvent('qb-fuel:station:server:updatebalance', "add", finalrefuelamount, StationBalance, CurrentLocation, FuelPrice)
								end
							end
						end
						local curfuel = GetFuel(vehicle)
			...
			if Config.Ox.Progress then
				if lib.progressCircle({
					...
					if RefuelingType == nil then
						if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel then
							TriggerServerEvent('qb-fuel:station:server:updatereserves', "remove", fuelamount, ReserveLevels, CurrentLocation)
							if CachedFuelPrice ~= nil then
								if Config.FuelDebug then
									print("We have a cached price: $"..CachedFuelPrice..", we will credit this to the gas station.")
								end
								TriggerServerEvent('qb-fuel:station:server:updatebalance', "add", fuelamount, StationBalance, CurrentLocation, CachedFuelPrice)
								CachedFuelPrice = nil
							else
								TriggerServerEvent('qb-fuel:station:server:updatebalance', "add", fuelamount, StationBalance, CurrentLocation, FuelPrice)
							end
						else
				...
				QBCore.Functions.Progressbar("refuel-car", Lang:t("prog_refueling_vehicle"), time, false, true, {
					...
					refueling = false
					if not Config.RenewedPhonePayment or purchasetype == "cash" then
						TriggerServerEvent('qb-fuel:server:PayForFuel', refillCost, purchasetype, FuelPrice)
					end
					local curfuel = GetFuel(vehicle)
					local finalfuel = (curfuel + fuelamount)
					if finalfuel > 99 and finalfuel < 100 then
...
RegisterNetEvent('qb-fuel:client:jerrycanfinalmenu', function(purchasetype)
	Moneyamount = nil
	...
	if Moneyamount > math.ceil(Config.JerryCanPrice + GlobalTax(Config.JerryCanPrice)) then
		TriggerServerEvent('qb-fuel:server:purchase:jerrycan', purchasetype)
	else
...
RegisterNetEvent('qb-fuel:client:purchasejerrycan', function()
	local playercashamount = QBCore.Functions.GetPlayerData().money['cash']
	if Config.Ox.Menu then
		lib.registerContext({
			id = 'purchasejerrycan',
			...
			options = {
				{
					...
					description = Lang:t("menu_pay_with_cash") .. playercashamount,
					icon = "fas fa-usd",
					event = 'qb-fuel:client:jerrycanfinalmenu',
					args = 'cash',
				},
				{
					title = Lang:t("menu_header_bank"),
			...
			if refuel and refuelAmount then
				...
				if Config.Ox.Progress then
					if lib.progressCircle({
						...
						TriggerServerEvent('qb-fuel:info', "remove", tonumber(refuelAmount), srcPlayerData, JerryCanItemData)
			...
			if refuel then
				...
				if Config.Ox.Progress then
					if lib.progressCircle({
						...
						local JerryCanItemData = data.itemData
						local srcPlayerData = QBCore.Functions.GetPlayerData()
						TriggerServerEvent('qb-fuel:info', "remove", tonumber(refuel.amount), srcPlayerData, JerryCanItemData)
	...
	if Config.Ox.Input then
		...
		if refuel then
			...
			if lib.progressCircle({
				...
				local srcPlayerData = QBCore.Functions.GetPlayerData()
				if Config.Ox.Inventory then
					TriggerServerEvent('qb-fuel:info', "add", tonumber(refuelAmount), srcPlayerData, 'jerrycan')
				else
					TriggerServerEvent('qb-fuel:info', "add", tonumber(refuelAmount), srcPlayerData, itemData)
				end
				
				if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel then
					TriggerServerEvent('qb-fuel:station:server:updatereserves', "remove", tonumber(refuelAmount), ReserveLevels, CurrentLocation)
					if CachedFuelPrice ~= nil then
						TriggerServerEvent('qb-fuel:station:server:updatebalance', "add", tonumber(refuelAmount), StationBalance, CurrentLocation, CachedFuelPrice)
					else
						TriggerServerEvent('qb-fuel:station:server:updatebalance', "add", tonumber(refuelAmount), StationBalance, CurrentLocation, FuelPrice)
					end
				else
					if Config.FuelDebug then print("Config.PlayerOwnedGasStationsEnabled == false or Config.UnlimitedFuel == true, this means reserves will not be changed.") end
				end
				local total = (tonumber(refuelAmount) * FuelPrice) + GlobalTax(tonumber(refuelAmount) * FuelPrice)
				TriggerServerEvent('qb-fuel:server:PayForFuel', total, "cash", FuelPrice)
			else 
				SetEntityVisible(fuelnozzle, true, 0)
				DeleteObject(JerrycanProp)
				StopAnimTask(ped, Config.JerryCanAnimDict, Config.JerryCanAnim, 1.0)
		...
		if refuel then
			...
			QBCore.Functions.Progressbar('refuel_gas', Lang:t("prog_jerry_can_refuel"), refueltimer, false,true, { -- Name | Label | Time | useWhileDead | canCancel
				...
				if Config.Ox.Inventory then
					...
					TriggerServerEvent('qb-fuel:info', "add", tonumber(refuelAmount), srcPlayerData, jerryCanData)
				end
				if RefuelingType == nil then	
					if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel then
						TriggerServerEvent('qb-fuel:station:server:updatereserves', "remove", tonumber(refuel.amount), ReserveLevels, CurrentLocation)
						if CachedFuelPrice ~= nil then
							TriggerServerEvent('qb-fuel:station:server:updatebalance', "add", tonumber(refuel.amount), StationBalance, CurrentLocation, CachedFuelPrice)
						else
							TriggerServerEvent('qb-fuel:station:server:updatebalance', "add", tonumber(refuel.amount), StationBalance, CurrentLocation, FuelPrice)
						end
				...
				TriggerServerEvent('qb-fuel:server:PayForFuel', total, "cash", FuelPrice, false, CachedFuelPrice)
			end, function() -- Play When Cancel
				SetEntityVisible(fuelnozzle, true, 0)
				DeleteObject(JerrycanProp)
				StopAnimTask(ped, Config.JerryCanAnimDict, Config.JerryCanAnim, 1.0)
						...
						if (tonumber(syphonAmount) <= tonumber(cargasamount)) then
							...
							if lib.progressCircle({
								...
								if GetFuel(vehicle) >= syphonAmount then
									...
									local srcPlayerData = QBCore.Functions.GetPlayerData()
									TriggerServerEvent('qb-fuel:info', "add", tonumber(syphonAmount), srcPlayerData, syphonData)
								else
						...
						if (tonumber(syphon.amount) <= tonumber(cargasamount)) then
							...
							QBCore.Functions.Progressbar('syphon_gas', Lang:t("prog_syphoning"), syphontimer, false, true, { -- Name | Label | Time | useWhileDead | canCancel
								...
								if GetFuel(vehicle) >= tonumber(syphon.amount) then
									...
									local syphonData = data.itemData
									local srcPlayerData = QBCore.Functions.GetPlayerData()
									TriggerServerEvent('qb-fuel:info', "add", tonumber(syphon.amount), srcPlayerData, syphonData)
									StopAnimTask(ped, Config.StealAnimDict, Config.StealAnim, 1.0)
								else
									QBCore.Functions.Notify(Lang:t("menu_syphon_vehicle_empty"), 'error')
								end
				...
				if Config.Ox.Input then
					...
					if refuel then
						...
						if lib.progressCircle({
							...
							local syphonData = data.itemData
							local srcPlayerData = QBCore.Functions.GetPlayerData()
							TriggerServerEvent('qb-fuel:info', "remove", tonumber(refuelAmount), srcPlayerData, syphonData)
						else
							StopAnimTask(ped, Config.JerryCanAnimDict, Config.JerryCanAnim, 1.0)
							QBCore.Functions.Notify(Lang:t("cancelled"), 'error')
						end
...
-- Helicopter Fueling --
RegisterNetEvent('qb-fuel:client:grabnozzle:special', function()
	local ped = PlayerPedId()
...
RegisterNetEvent('qb-fuel:client:returnnozzle:special', function()
	HoldingSpecialNozzle = false
	TriggerServerEvent("InteractSound_SV:PlayOnSource", "putbacknozzle", 0.4)
	Wait(250)
	DeleteObject(SpecialFuelNozzleObj)
						...
						while PlayerInSpecialFuelZone do
							...
							if IsControlJustReleased(0, Config.AirAndWaterVehicleFueling['refuel_button']) --[[ Control in Config ]] then
								...
								elseif IsPedInAnyVehicle(PlayerPedId(), true) then 
									...
									TriggerEvent('qb-fuel:client:RefuelMenu', 'special')
								end
							end
						end
					end)
	...
	if Config.TargetResource == 'ox_target' then
		local options = {
			[1] = {
				...
				event = 'qb-fuel:client:RefuelMenu'
			},
			[2] = {
				name = 'qb-fuel:options:2',
				icon = "fas fa-bolt",
				...
				event = "qb-fuel:client:electric:RefuelMenu",
			}
		}
		...
		exports['qb-target']:AddTargetModel(props, {
			options = {
				...
				},
				{
					num = 2,
					type = "client",
					event = "qb-fuel:client:purchasejerrycan",
					icon = "fas fa-fire-flame-simple",
				...
				},
				{
					num = 4,
					type = "client",
					event = "qb-fuel:client:grabnozzle:special",
					icon = "fas fa-gas-pump",
				...
				},
				{
					num = 5,
					type = "client",
					event = "qb-fuel:client:returnnozzle:special",
					icon = "fas fa-hand",
			...
			options = {
				{
					type = "client",
					action = function ()
						TriggerEvent('qb-fuel:client:RefuelMenu')
					end,
					icon = "fas fa-gas-pump",
				...
				},
				{
					type = "client",
					action = function()
						TriggerEvent('qb-fuel:client:electric:RefuelMenu')
					end,
					icon = "fas fa-bolt",
