if Config.PlayerOwnedGasStationsEnabled then -- This is so Player Owned Gas Stations are a Config Option, instead of forced. Set this option in shared/config.lua!
        ...
        if not newLabel or newLabel == nil then
            ...
            MySQL.Async.fetchAll('SELECT label FROM fuel_stations WHERE location = ?', {location}, function(result)
                if result then
                    ...
                    TriggerClientEvent('qb-fuel:client:updatestationlabels', -1, location, newLabel)
                else
            ...
            if src then
                TriggerClientEvent('qb-fuel:client:updatestationlabels', src, location, newLabel)
            else
                TriggerClientEvent('qb-fuel:client:updatestationlabels', -1, location, newLabel)
            end
        end
    ...
    -- Events
    RegisterNetEvent('qb-fuel:server:updatelocationlabels', function()
        local src = source
        local location = 0
    ...
    RegisterNetEvent('qb-fuel:server:buyStation', function(location, CitizenID)
        local src = source
    ...
    RegisterNetEvent('qb-fuel:stations:server:sellstation', function(location)
        local src = source
        local Player = QBCore.Functions.GetPlayer(src)
    ...
    RegisterNetEvent('qb-fuel:station:server:Withdraw', function(amount, location, StationBalance)
        local src = source
        local Player = QBCore.Functions.GetPlayer(src)
    ...
    RegisterNetEvent('qb-fuel:station:server:Deposit', function(amount, location, StationBalance)
        local src = source
        local Player = QBCore.Functions.GetPlayer(src)
        local setamount = (StationBalance + amount)
    ...
    RegisterNetEvent('qb-fuel:stations:server:Shutoff', function(location)
        local src = source
    ...
    RegisterNetEvent('qb-fuel:station:server:updatefuelprice', function(fuelprice, location)
        local src = source
    ...
    RegisterNetEvent('qb-fuel:station:server:updatereserves', function(reason, amount, currentlevel, location)
    ...
    RegisterNetEvent('qb-fuel:station:server:updatebalance', function(reason, amount, StationBalance, location, FuelPrice)
    ...
    RegisterNetEvent('qb-fuel:stations:server:buyreserves', function(location, price, amount)
        ...
        if ReserveBuyPossible and Player.Functions.RemoveMoney("bank", price, "Purchased"..amount.."L of Reserves for: "..Config.GasStations[location].label.." @ $"..Config.FuelReservesPrice.." / L!") then
            if not Config.OwnersPickupFuel then
                ...
                TriggerClientEvent('qb-fuel:station:client:initiatefuelpickup', src, amount, NewAmount, location)
    ...
    RegisterNetEvent('qb-fuel:station:server:fuelpickup:failed', function(location)
        local src = source
        if location then
            if FuelPickupSent[location] then
                ...
                if Config.FuelDebug then
                    print("`qb-fuel:station:server:fuelpickup:failed` | FuelPickupSent[location] is not valid! Location: "..location)
                end
    ...
    RegisterNetEvent('qb-fuel:station:server:fuelpickup:finished', function(location)
        local src = source
        if location then
            if FuelPickupSent[location] then
                local cid = QBCore.Functions.GetPlayer(src).PlayerData.citizenid
    ...
    RegisterNetEvent('qb-fuel:station:server:updatelocationname', function(newName, location)
        local src = source
        ...
        TriggerClientEvent('qb-fuel:client:updatestationlabels', -1, location, newName)
    end)

    -- Callbacks 
    QBCore.Functions.CreateCallback('qb-fuel:server:locationpurchased', function(source, cb, location)
    ...
    QBCore.Functions.CreateCallback('qb-fuel:server:doesPlayerOwnStation', function(source, cb)
        local src = source
        local Player = QBCore.Functions.GetPlayer(src)
    ...
    QBCore.Functions.CreateCallback('qb-fuel:server:isowner', function(source, cb, location)
        local src = source
        local Player = QBCore.Functions.GetPlayer(src)
        local citizenid = Player.PlayerData.citizenid
        if Config.FuelDebug then print("working on it.") end
    ...
    QBCore.Functions.CreateCallback('qb-fuel:server:fetchinfo', function(source, cb, location)
        local src = source
        local Player = QBCore.Functions.GetPlayer(src)
    ...
    QBCore.Functions.CreateCallback('qb-fuel:server:checkshutoff', function(source, cb, location)
        if Config.FuelDebug then print("Fetching Shutoff State for Location: "..location) end
        cb(Config.GasStations[location].shutoff)
	end)
    
    QBCore.Functions.CreateCallback('qb-fuel:server:fetchlabel', function(source, cb, location)
        if Config.FuelDebug then print("Fetching Shutoff State for Location: "..location) end
        MySQL.Async.fetchAll('SELECT label FROM fuel_stations WHERE location = ?', {location}, function(result)
            if result then
                cb(result)
