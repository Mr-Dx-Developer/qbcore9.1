...
if Config.RenewedPhonePayment then
	RegisterNetEvent('qb-fuel:server:phone:givebackmoney', function(amount)
		local src = source
...
RegisterNetEvent("qb-fuel:server:OpenMenu", function(amount, inGasStation, hasWeapon, purchasetype, FuelPrice)
	local src = source
	...
	if inGasStation == true and not hasWeapon then
		if Config.RenewedPhonePayment and purchasetype == "bank" then
			TriggerClientEvent("qb-fuel:client:phone:PayForFuel", src, amount)
		else
			if Config.Ox.Menu then
				if Config.FuelDebug then print("going to open the context menu (OX)") end
				TriggerClientEvent('qb-fuel:client:OpenContextMenu', src, total, amount, purchasetype)
			else
...
RegisterNetEvent("qb-fuel:server:PayForFuel", function(amount, purchasetype, FuelPrice, electric)
	local src = source
	if not src then return end
	local Player = QBCore.Functions.GetPlayer(src)
	if not Player then return end
...
RegisterNetEvent("qb-fuel:server:purchase:jerrycan", function(purchasetype)
