...
-- Events
RegisterNetEvent("qb-fuel:server:electric:OpenMenu", function(amount, inGasStation, hasWeapon, purchasetype, FuelPrice)
	local src = source
	...
	if inGasStation and not hasWeapon then
		if Config.RenewedPhonePayment and purchasetype == "bank" then
			TriggerClientEvent("qb-fuel:client:electric:phone:PayForFuel", src, amount)
		else
