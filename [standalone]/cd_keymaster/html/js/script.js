...
function showKeymaster(){
    let promise = new Promise((resolve, reject) => {
        score = 0;
        $('#keymaster-container').css({'display':'block'})
...
function endGame(status, message){
    ...
    $('#keymaster-container').css({'display':'none'})
}
async function nextKey(i){
    ...
    keys[i].shown = true;
    $(`[data-kid='${i}']`).css("right", "-75px").css("background-color", "#eee").show(200);
    keys[i].right = -75;
}
...
async function evalKey(key){
    for(let i = 0; i < keys.length; i++){
        if(keys[i].key == key && keys[i].shown){
            if(keys[i].right+75 <= settings.boxL && keys[i].right+75 >= settings.boxR){
                settings.speed += settings.speedIncrement;
                keys[i].shown = false;
                updateScore(keys[i].right+75 - settings.boxR);
    ...
    game = window.setInterval(() => {
        for(let i = 0; i < keys.length; i++){
            if(keys[i].shown){
                ...
                if(keys[i].right >= settings.boxR+75){
                    keys[i].shown = false;
                    updateScore(-50);
