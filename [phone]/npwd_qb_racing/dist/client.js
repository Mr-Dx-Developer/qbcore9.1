(() => {
  ...
  RegisterNuiCB("npwd:qb-racing:setupRace" /* SetupRace */, (data, cb) => {
    emitNet("qb-lapraces:server:SetupRace" /* SetupRace */, data.trackId, data.laps);
    cb({ status: "ok" });
  });
  RegisterNuiCB("npwd:qb-racing:joinRace" /* JoinRace */, (data, cb) => {
    emitNet("qb-lapraces:server:JoinRace" /* JoinRace */, { RaceName: data.raceName, RaceId: data.raceId });
    cb({ status: "ok" });
  });
  ...
  onNet("qb-phone:client:UpdateLapraces", () => {
    npwdExports.sendUIMessage("npwd:qb-racing:updateData" /* UpdateData */);
  });
